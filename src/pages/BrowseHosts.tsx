...
import { GAME_MIN_VRAM } from '../constants';
import { 
  Gamepad2, Cpu, Database, Wifi, Coins, Search, History, Check, ShieldAlert, Zap, Radio, Loader2, ArrowRight
} from 'lucide-react';

const POPULAR_GAMES = [
  { name: 'Cyberpunk 2077', vram: 8, banner: '👾 Action RPG / Heavy' },
  { name: 'Elden Ring', vram: 6, banner: '⚔️ Dark Fantasy / High' },
  { name: 'Counter-Strike 2', vram: 4, banner: '🔫 Competitive Shooter / Fast' },
  { name: 'Dota 2', vram: 2, banner: '🧙 MOBA Arena / Medium' },
  { name: 'Alan Wake 2', vram: 12, banner: '🔦 Survival Horror / Extreme' },
];
...
export default function BrowseHosts() {
  ...
  const syncLobby = async (gameToQuery: string) => {
    try {
      setIsLoading(true);
      setErrorMsg('');
      ...
      // Fetch hosts matching selected game requirements
      const hostsRes = await hostApi.getHosts(gameToQuery);
  ...
  const handleCustomGameSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (!customGame.trim()) return;
    setSelectedGame(customGame);
    syncLobby(customGame);
  };

  useEffect(() => {
    syncLobby(selectedGame);
  ...
  const handleConfirmBooking = async (hostId: number) => {
    ...
    try {
      ...
      const gameName = customGame.trim() ? customGame : selectedGame;
      const res = await sessionApi.book({ host_id: hostId, game_name: gameName });
  ...
  const getRequiredVram = () => {
    return GAME_MIN_VRAM[selectedGame] || 4;
  };
        ...
        {activeSession && (
          <div className="bg-gradient-to-r from-violet-950 to-indigo-950 border border-violet-800 p-6 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-2xl">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-white tracking-tight">Active Pipeline Connection Running</h2>
                <p className="text-sm text-slate-400 mt-0.5">
                  You are currently sharing <span className="font-bold text-white">{activeSession.gpu_model}</span> running <span className="text-indigo-400 font-bold">{activeSession.game_name}</span>.
                </p>
              </div>
            </div>
            <button 
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {POPULAR_GAMES.map((game) => (
              ...
              >
                <p className="text-[10px] font-mono font-bold text-indigo-400 uppercase tracking-tight">{game.banner}</p>
                <h3 className="text-white font-bold text-sm mt-1 truncate">{game.name}</h3>
                <div className="flex items-center gap-1.5 mt-3 text-xs text-slate-400">
                  <Database className="w-3.5 h-3.5 text-indigo-400" />
                  <span>Req. VRAM: <span className="font-bold font-mono text-white">{game.vram}GB</span></span>
                </div>
              </button>
            ))}
          </div>

          {/* Search bar custom games */}
          <form onSubmit={handleCustomGameSearch} className="flex gap-3 bg-[#131a2c] p-3 rounded-2xl border border-slate-800/80 max-w-lg">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-3 h-4.5 w-4.5 text-slate-500" />
              <input
                id="search-input"
                type="text"
                placeholder="Or input custom software game name..."
                value={customGame}
                onChange={(e) => setCustomGame(e.target.value)}
                className="w-full pl-10 pr-3 py-2 bg-transparent border-0 text-sm focus:outline-none text-slate-200 placeholder-slate-500"
              />
            </div>
            <button 
              type="submit"
              className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold font-mono px-4 py-2 rounded-xl transition"
            >
              Check Match
            </button>
          </form>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-sm font-semibold tracking-wider uppercase text-slate-400 font-mono">
                2. Matching GPU Host Nodes ({hosts.length})
              </h2>
              
              <div className="text-xs text-slate-400 font-mono bg-[#131a2c] border border-slate-800 px-3 py-1 rounded-full">
                Filtering VRAM &ge; {getRequiredVram()}GB
              </div>
            </div>
            ...
            {isLoading ? (
              <div className="bg-[#131a2c] border border-slate-850 p-12 rounded-2xl text-center space-y-3">
                ...
                <p className="text-sm font-mono text-slate-400">Running matchmaking matching contracts...</p>
              </div>
            ) : hosts.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        {host.user_id === user?.id ? (
                          <div className="bg-indigo-600/10 border border-indigo-505 px-3 py-1.5 rounded-xl text-center text-[10px] font-bold font-mono text-indigo-400">
                            Your Node Device
                          </div>
                        ) : (
                          <button
              ...
              <div className="bg-[#131a2c] border border-slate-850 p-12 rounded-2xl text-center">
                ...
                <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1 leading-relaxed">
                  There are no hosts matching target parameters or sufficient VRAM for running "{selectedGame}" right now. Try select another software or wait!
                </p>
              </div>
            )}
          </div>
